...
uint8_t keyboardhid[8];

uint8_t prevState = GPIO_PIN_SET;

/* USER CODE END 0 */

/* USER CODE BEGIN 2 */

void SendShiftPress(uint8_t press) {

    memset(keyboardhid, 0, sizeof(keyboardhid));
    // Shift key press
    keyboardhid[2] = press ? 0x04 : 0x00;
...
int main(void)
{
    ...
    while (1)
    {
        uint8_t currentState = HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_11);
        ...
        if (currentState == GPIO_PIN_SET && prevState == GPIO_PIN_RESET) {
